...
(function (window) {
    ...
    const setupGeetest = function () {
        return new Promise((res) => {
            ...
            window.initGeetest4(
                {
                    captchaId: "e000881b946cad6dcc39aa1eb40c80b0",
                    product: "bind",
                    protocol: "https://",
                    hideSuccess: true,
                    mask: {
    ...
    const validateGeetest = async function () {
        const gt = await setupGeetest();
        if (!gt) return null;
        return new Promise(res => {
            gt.onNextReady(() => {
                gt.showCaptcha();
    ...
    window.fetch = new Proxy(originalFetch, {
        apply: async function (target, thisArg, argumentsList) {
            ...
            if (typeof url === 'string' && url.endsWith('/v1/user/request-verification-code')) {
                const body = JSON.parse(options.body);
                if (!body.captcha_validate) {
                    const captcha_validate = await validateGeetest();
                    if (!captcha_validate) {
                        throw new Error('Get Verification Code Failed');
                    }
                    body.captcha_validate = captcha_validate;
                    options.body = JSON.stringify(body);
                }
            }
            return originalFetch.apply(thisArg, argumentsList);
        }
    });
    ...
    window.XMLHttpRequest = function () {
        ...
        xhr.send = function (data) {
            ...
            if (request.method === 'POST' && request.url.endsWith('/v1/user/request-verification-code')) {
                try {
                    const body = JSON.parse(data);
                    if (!body.captcha_validate) {
                        validateGeetest().then(captcha_validate => {
                            if (!captcha_validate) {
                                xhr.abort();
                            ...
                            body.captcha_validate = captcha_validate;
                            originalSend.call(xhr, JSON.stringify(body));
                        })
                        return;
                    }
                } catch (e) {
