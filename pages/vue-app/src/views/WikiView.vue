<template>
  <section class="legacy-archive wiki-view">
    ...
    <header class="archive-marquee wiki-marquee"><span class="ritual-label">REVERSE ARCHIVE / 1999</span><h1>Wiki 档案馆</h1><p>这里不是百科首页，而是伪人大本营的卷宗柜：表界、里界、伪人、哨站、伪物和活动记录会按层级归档。投稿审核通过后，伪物类条目会进入图鉴数据库并返还背包奖励。</p></header>

    <form v-if="editorOpen" class="wiki-submit-panel wiki-editor-panel" @submit.prevent="submit">
      <div class="wiki-form-grid"><label>分类名称<input v-model.trim="draft.target" list="wiki-cats" placeholder="世界观 / 里界 / 伪物图鉴"><datalist id="wiki-cats"><option v-for="c in categories" :key="c.name" :value="c.name" /></datalist></label><label>编辑类型<select v-model="draft.submit_type"><option>新增词条</option><option>修订词条</option><option>新建分类</option><option>图鉴投稿</option></select></label><label>条目名称<input v-model.trim="draft.title" placeholder="档案标题"></label></div>
      <label>正文内容<textarea v-model.trim="draft.content" rows="6" placeholder="写条目正文、修订内容、设定说明。伪物图鉴建议写：发现地点、等级、收容措施、特殊效果、副作用。"></textarea></label>
    ...
    <div class="wiki-hierarchy" :class="{'is-root':!openCat && !query}">
      ...
      <section class="wiki-drill-panel">
        <header class="drill-head"><div><span class="ritual-label">REVERSE ARCHIVE / 1999</span><h2>{{ openCat || (query ? '搜索结果' : '选择档案卷宗') }}</h2><p>{{ openCat ? categoryHint(openCat) + ' / 当前栏目记录' : '从左侧选择卷宗；移动端可直接向下浏览分类。' }}</p></div><button v-if="openCat" class="back-note" @click="openCat=''">返回卷宗</button></header>
...
const draft=reactive({target:'伪物图鉴',submit_type:'新增词条',title:'',content:'',images:[]})
