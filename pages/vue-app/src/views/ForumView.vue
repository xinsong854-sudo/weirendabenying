...
const props=defineProps({session:String}); const posts=ref([]), text=ref(''), images=ref([]), pickerOpen=ref(false), library=ref(loadLib()), cards=ref([]), roleId=ref('')
...
async function post(){if(!text.value&&!images.value.length)return; await siteApi('/api/forum/posts',{method:'POST',headers:sessionHeaders(props.session),body:JSON.stringify({channel:'主论坛',content:text.value,images:images.value,role_card_id:roleId.value})}); text.value=''; images.value=[]; pickerOpen.value=false; await load()}
